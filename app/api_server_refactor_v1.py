...
def _round_df_columns(df: pd.DataFrame, cols: list[str], decimals: int = 2) -> pd.DataFrame:
    if df is None or df.empty:
        return df
    df = df.copy()
    for col in cols:
...
def _fetch_ga4_product_source_df(start_date: str, end_date: str) -> pd.DataFrame:
    ...
    for row in ga4_response.rows:
        ga4_rows.append({
            ...
            'item_revenue': round(float(row.metric_values[3].value), 2),
        })
...
def _pmax_campaign_products_impl(f: Dict[str, Any]) -> Dict[str, Any]:
    try:
        ...
        products_result = _google_ads_products_impl({
        ...
        rows = products_result.get('rows', []) if isinstance(products_result, dict) else []
        df = pd.DataFrame(rows)
        pmax_campaign_names = _get_pmax_campaign_names(ads_date_range)

        if df.empty or not pmax_campaign_names:
            return {
                ...
                'metrics': {
                    ...
                    'value': 0.0,
                    'roas': 0.0,
                    'campaign_count': len(pmax_campaign_names),
                    'product_count': 0,
                },
                'rows': [],
                'insights': [],
                ...
                'error': '' if pmax_campaign_names else 'Nu am găsit campanii PERFORMANCE_MAX în Google Ads pentru intervalul selectat.',
            }

        rename_map = {}
        if 'product_name' in df.columns and 'item_name' not in df.columns:
            rename_map['product_name'] = 'item_name'
        if 'product_title' in df.columns and 'item_name' not in df.columns:
            rename_map['product_title'] = 'item_name'
        if 'product_item_id' in df.columns and 'item_id' not in df.columns:
            rename_map['product_item_id'] = 'item_id'
        if 'value' in df.columns and 'conversion_value' not in df.columns:
        ...
        df = df[df['campaign_name'].isin(pmax_campaign_names)].copy()

        if pmax_campaign_name:
            df = df[df['campaign_name'] == pmax_campaign_name].copy()
        if campaign_filter:
            df = df[df['campaign_name'].str.lower().str.contains(campaign_filter, na=False)].copy()
        if product_filter:
            df['item_name'] = df['item_name'].fillna('').astype(str)
            df = df[df['item_name'].str.lower().str.contains(product_filter, na=False)].copy()
        if selected_item_id:
            df['item_id'] = df['item_id'].astype(str)
            df = df[df['item_id'] == selected_item_id].copy()
        ...
        if df.empty:
            return {
                ...
                'campaign_type': 'PERFORMANCE_MAX',
                'identification_method': 'channel_type',
                'error': 'Există campanii PERFORMANCE_MAX, dar nu am găsit produse pentru filtrele selectate.',
            }
        ...
        product_count = int(df['item_id'].astype(str).nunique()) if 'item_id' in df.columns else len(df)

        insights = []
        cost_no_conv = df[(df['cost'] > 0) & (df['conversions'] <= 0)]
        ...
        scalable = df[(df['conversions'] > 0) & (df['roas'] >= 3)]
        ...
        weak_ctr = df[(df['impressions'] > 1000) & (df['ctr'] < 0.5)]
        ...
        df = df.sort_values(['conversion_value', 'cost'], ascending=[False, False])
        return {
            'campaigns': sorted([str(x) for x in pmax_campaign_names]),
            'metrics': {k: to_python_scalar(v) for k, v in {
                'cost': total_cost,
            ...
            'rows': to_python_records(df.fillna(0).to_dict(orient='records')),
...
def _ga4_products_impl(f: Dict[str, Any]) -> Dict[str, Any]:
    ...
    df = _round_df_columns(df, ['item_revenue'], 2)
...
def _ga4_traffic_impl(f: Dict[str, Any]) -> Dict[str, Any]:
    ...
    df = _round_df_columns(df, ['purchase_revenue', 'revenue_per_session'], 2)
...
def _product_channels_impl(f: Dict[str, Any]) -> Dict[str, Any]:
    ...
    grouped = _round_df_columns(grouped, ['item_revenue', 'revenue_per_view'], 2)
